...
$sinab->get('/medicamentosplanificacionprorroga', function () use ($app) {
	 ...
	 $acceso = $app['autentica'];
	 if (!$acceso($app, $_GET["tocken"])){ return $app->json($error, 404); }
	 $sql = "SELECT DISTINCT PP.IDPRODUCTO,C.IDCONTRATO, C.NUMEROCONTRATO, C.IDPROVEEDOR, PV.NOMBRE, PPE.IDESTABLECIMIENTO, PPE.CANTIDADALMACEN, PPE.CANTIDADCOMPRAR,PC.PRECIOUNITARIO
