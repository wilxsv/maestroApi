...
//Listado de medicamentos en una estimacion por id de programacion
 $sinab->get('/medicamentosestimacion', function () use ($app) {
	 ...
	 $acceso = $app['autentica'];
	 if (!$acceso($app, $_GET["tocken"])){ return $app->json($error, 404); }
	 $sql = "WITH OBJ(IDPRODUCTO, NUMEROCONTRATO, IDPROVEEDOR) AS (SELECT DISTINCT PC.IDPRODUCTO,C.NUMEROCONTRATO, PC.IDPROVEEDOR FROM SAB_UACI_CONTRATOS AS C INNER JOIN SAB_UACI_PRODUCTOSCONTRATO AS PC ON PC.IDCONTRATO=C.IDCONTRATO AND PC.IDPROVEEDOR = C.IDPROVEEDOR AND PC.IDESTABLECIMIENTO = C.IDESTABLECIMIENTO WHERE C.NUMEROMODALIDADCOMPRA ='$licitacion' and C.IDTIPODOCUMENTO=2 AND  C.IDPROVEEDOR = $proveedor 
INTERSECT SELECT DISTINCT PP.IDPRODUCTO,C.NUMEROCONTRATO, C.IDPROVEEDOR FROM  SAB_URMIM_PROGRAMACIONPRODUCTO PP INNER JOIN SAB_UACI_PRODUCTOSCONTRATO PC ON PC.IDPRODUCTO = PP.IDPRODUCTO INNER JOIN SAB_UACI_CONTRATOS C 
ON PC.IDCONTRATO=C.IDCONTRATO AND PC.IDPROVEEDOR = C.IDPROVEEDOR AND PC.IDESTABLECIMIENTO = C.IDESTABLECIMIENTO WHERE PP.IDPROGRAMACION = $programacion )
